...
void main() {
    ...
    // For circular container, we want to cover the entire circle
    // Calculate scale to ensure the image covers the circle completely
    float scale = max(1.0 / imageAspect, imageAspect);
    
    // Center and scale UVs for proper cover behavior
    ...
    if (uGrayscale) {
        ...
        outColor = vec4(gray, gray, gray, texColor.a);
    } else {
        outColor = texColor;
    }
    outColor.a *= vAlpha;
...
class InfiniteGridMenu {
  ...
  private init(onInit?: InitCallback): void {
    ...
    this.discLocations = {
      ...
      uGrayscale: gl.getUniformLocation(this.discProgram!, "uGrayscale"),
    };
  ...
  private render(): void {
    ...
    gl.uniform1i(this.discLocations.uGrayscale, this.isGrayscale ? 1 : 0);
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  useEffect(() => {
    if (sketchRef.current) {
      sketchRef.current.isGrayscale = isGrayscale;
    }
  }, [isGrayscale]);
  ...
  return (
    <div className="relative w-full h-full">
      <canvas
        ...
        ref={canvasRef}
        className="cursor-grab w-full h-full overflow-hidden relative outline-none active:cursor-grabbing backdrop-blur-sm bg-white/5 ring ring-black/10 rounded-lg shadow-md"
      />
      ...
      {activeItem && (
        ...
        `}
          ...
          {/* Description */}
          <div className="hidden md:flex select-none absolute  z-10  flex-col items-center justify-center   right-[15%] top-1/2 -translate-y-1/2">
            <p
              className={`
              select-none
              max-w-[10ch]
            ...
            <div
              ...
              className={`
                  ...
                  hover:scale-110
                  mt-2
                  ${isGrayscale ? "text-gray-600" : "text-blue-500"}
                  ${
                    isMoving
                      ? "opacity-0 pointer-events-none duration-[100ms]"
                      : "opacity-100 pointer-events-auto duration-[500ms]"
          ...
          <div className="md:hidden absolute top-10  w-1/3 translate-x-3/3 ">
            <p className="text-xs  select-none text-center">
              Desliza para ver más
            </p>
          </div>

          {/* Color Toggle Button - Mobile */}
          <div
            onClick={toggleGrayscale}
            className={`
               md:hidden
